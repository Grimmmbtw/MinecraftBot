...
// Message de join
client.on('guildMemberAdd', async member => {
  const channel = member.guild.channels.cache.get('1093943502225801331'); // Mettre l'id du channel pour le message de bienvenue
  const invites = await member.guild.invites.fetch();
  const invite = invites.find(i => i.inviter && i.uses > (i.inviter.uses || 0));
  const serveur = 'https://discord.gg/shandera'
  ...
  const join = new MessageEmbed()
    ...
    .setURL(serveur)
    .setDescription(`**${member.user.tag}** vient de rejoindre l'aventure !\n${invite ? `Il a été invité par **${invite.inviter.tag}**` : 'Nous ne savons pas qui l\'a invité.'} \n\nLe discord compte désormais **${member.guild.memberCount}** aventuriers !`)
    .setThumbnail('https://cdn.discordapp.com/attachments/1093653326240948257/1093992838246711356/a8c63134c7710ccc014c9ba7b5f94e75.png')
    .setFooter(`© Shandera - 2022`)
    .setTimestamp();
...
client.on('guildMemberRemove', async member => {
  const channel = member.guild.channels.cache.get('1093992746722795541'); // Mettre l'id du channel pour le message de leave
  const serveur = 'https://discord.gg/shandera'

  const leave = new MessageEmbed()
  .setColor('#e67e22')
    .setTitle(`À quitter Shandera ☹️`)
    .setURL(serveur)
